...
router.post(
  ...
  (req, res) => {
    ...
    setTimeout(() => {
      try {
        setTimeout(() => {
          Post.findById(idOfPost).then((post) => {
            post.remove().then(() => {
              console.log("deleted");
            });
          });
...
router.post(
  ...
  (req, res) => {
    Post.findById(req.params.id)
      .then((post) => {
        if (
          post.likes.filter((like) => like.userdata.toString() === req.user.id)
            .length > 0
        ) {
          return res
            .status(400)
      ...
      .then((post) => {
        if (
          ...
          const removeIndex = post.likes
            .map((item) => item.userdata.toString())
            .indexOf(req.user.id);
        ...
        if (
          ...
          post.comments.filter((comment) => {
            if (comment._id == req.params.comment_id) {
              if (
                comment.likes.filter((like) => like.userdata == req.user.id)
                  .length > 0
              ) {
